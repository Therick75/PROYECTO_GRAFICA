...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(800, 600, "Múltiples Cubos", NULL, NULL);
    ...
    if (data) { glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, w, h, 0, GL_RGB, GL_UNSIGNED_BYTE, data); glGenerateMipmap(GL_TEXTURE_2D); }
    stbi_image_free(data);

    glUseProgram(shaderProgram); // Lo activamos una vez fuera del bucle
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Matrices globales (Vista y Proyección)
